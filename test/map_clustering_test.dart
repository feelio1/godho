...
void main() {
  test('cellSizeForZoom shrinks as zoom increases and disables at the cap', () {
    expect(cellSizeForZoom(17), 0);
    expect(cellSizeForZoom(20), 0);
    expect(cellSizeForZoom(16), greaterThan(0));
    expect(cellSizeForZoom(10), greaterThan(cellSizeForZoom(16)));
    expect(cellSizeForZoom(5), greaterThan(cellSizeForZoom(10)));
  });
  ...
  test('at/above the disabled zoom, every hospital is its own group', () {
    ...
    final groups = clusterHospitals(hospitals, 17);
  ...
  test('nearby hospitals merge into one cluster group when zoomed out', () {
    ...
    final groups = clusterHospitals(hospitals, 10);
  ...
  test('every hospital appears in exactly one group regardless of zoom', () {
    ...
    for (final zoom in [5, 8, 11, 14, 16, 17, 19]) {
