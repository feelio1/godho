...
- 마커는 캐릭터(장구름) 이미지 자리입니다. 실제 이미지가 없어 지금은 중립 placeholder
  ...
  그 이미지로 자동 교체됩니다(마스코트와 동일한 패턴). 병원 이름 라벨은 일정 줌 레벨 이상으로
  확대했을 때만 보여 겹침을 막습니다.
...
- 카카오맵 SDK 자체에는 클러스터링 기능이 없어, `lib/utils/map_clustering.dart`에서 줌
  레벨에 따라 격자 크기를 바꿔가며 가까운 병원들을 직접 묶습니다. 줌아웃 상태에서는 묶인
  개수("N")가 표시되고, 탭하면 확대되며 개별 마커로 풀립니다.
- 지도 우하단의 버튼으로 현재 위치로 이동할 수 있습니다. 위치 권한이 없으면 버튼을 눌렀을 때
