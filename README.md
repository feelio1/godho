...
- 병원 상세 화면 헤더의 압정 아이콘으로 병원을 "지정(단골)"으로 등록/해제할 수 있습니다. 지정한
  병원은 홈 상단 "지정 병원" 섹션에 카드로 뜨고, 카드에서 바로 전화·길찾기·상세보기가 가능합니다.
  "저장(♡)"과는 별개 목록입니다. 영업시간·실시간 영업여부는 표시하지 않습니다(데이터 없음 —
  확인이 필요하면 상세의 외부 링크로 안내).
- 홈의 "우리 아이 건강기록" 카드를 누르면 반려동물 프로필과 진료 기록을 남길 수 있는 화면으로
  이동합니다(하단 탭을 늘리지 않고 홈 진입점으로만 제공). 이름·종·품종·생일·사진을 등록하고,
  방문할 때마다 날짜·병원(검색해서 선택하거나 직접 입력)·메모·몸무게·진료비·사진을 기록으로
  남길 수 있습니다. 기록 개수·총 진료비·최근 몸무게만 텍스트로 요약하며, 그래프나 "비만/정상"
  같은 판정은 제공하지 않습니다(CLAUDE.md 평가 금지 원칙).
- 이 기록은 계정·서버 없이 기기에만 저장됩니다(SharedPreferences) — 앱을 지우거나 기기를
  바꾸면 사라질 수 있다는 안내가 화면에 항상 표시됩니다. 데이터 모델(Pet/MedicalRecord)은
  순수 클래스 + JSON 직렬화로 분리되어 있어, 추후 로그인·클라우드 동기화가 추가되어도 저장소만
  교체하면 됩니다.
