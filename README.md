...
홈은 검색창 → 배너 → 지정 병원(있을 때) → 지도 바로가기/지역 표시 → "내 주변 가까운 병원"
(세로 카드 5곳 + "전체 병원 보기") → "운영 20년 이상 병원" → "최근 개원한 병원" → "최근 확인한
병원" → "저장한 병원" 순으로, 데이터가 있는 섹션만 세로로 쌓여 스크롤됩니다. 섹션 제목은 전부
정렬 기준을 사실로만 표기하며("가까운 순", "운영 20년 이상", "최근 개원 순"), "추천·베스트·믿을
수 있는" 같은 평가 표현이나 광고·커머스·평점 기능은 넣지 않습니다(CLAUDE.md 평가 금지 원칙). 위치
정보가 없으면 "내 주변 가까운 병원"은 선택 지역 기준 목록을 그대로 보여줍니다.

## 뒤로가기 동작

시스템 뒤로가기는 상세 등 push된 화면에서는 원래 화면(목록 등)으로 돌아가고, 하단 탭의 목록
화면(주변 병원/진료기록/저장)에서는 먼저 홈 탭으로 이동합니다. 홈 탭에서 한 번 더 누르면 "한 번
더 누르면 종료됩니다" 안내가 뜨고, 2초 안에 다시 누르면 그때 앱이 종료됩니다 — 실수로 바로
종료되지 않게 하기 위함입니다(`lib/screens/main_shell.dart`의 `PopScope`).
