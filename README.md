...
- 같은 탭에서 진료 예약도 등록할 수 있습니다. 예약마다 "며칠 전 + 몇 시"로 알림을 원하는 만큼
  추가할 수 있고(예: 전날 저녁 + 당일 아침), 그 시각에 기기 로컬 알림이 울립니다. 알림 권한은
  예약에 알림을 처음 추가해 저장하는 시점에만 요청하며, 권한을 거부하거나 알림 기능을 쓸 수 없는
  기기에서도 앱은 항상 정상 동작합니다(예약 기록 자체는 알림과 무관하게 남습니다).
